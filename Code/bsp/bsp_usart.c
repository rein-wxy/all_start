...
/*遥控器*/
uint8_t remote_control_lever_buff[100];

void Callback_REFEREE_Handle(unsigned char *data,uint8_t* buff)
{
	for(int i=0;i<200;i++)
	{
		uart6_rx_buff[i] = buff[i];
	}
...
void HAL_UART_IDLE_IRQHandler(UART_HandleTypeDef *huart)
{
	...
	//接收空闲中断
	if(huart == &huart3)
	    Callback_RC_Handle(&RC_DR16,huart->pRxBuffPtr);
